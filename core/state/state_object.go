...
// @anson
// stateObject是Ethereum里用来管理一个账户所有信息修改的结构体
// 它以一个Address类型变量为唯一标识符
// StateDB在内部用一个巨大的map结构来管理这些stateObject对象
// 所有账户信息：包括Ether余额，指令数组Code，该账户发起合约次数nonce等，他们所有的变化，会首先缓存到stateDB里的某个stateObject里
// 然后在适当的时候，被StateDB一起提交到底层数据库
// 一个contract所对应的stateObject地址，是Contract的self地址，也就是转账的转入地址

// EVM目前有五个函数可以创建并执行Contract，按照调用方式，可以分成两类：
// 1: Create(), Call()二者均在StateProcessor的ApplyTransaction()被调用以执行单个交易，并且都有调用转账函数完成转账
// 2: CallCode(), DelegateCode(), StaticCall()三者由于分别对应不同的虚拟机指令(1 byte)操作，不会用以执行单个交易，也都不能处理转账
...
type stateObject struct {
	...
	// @anson
	// 存放一种被称为State的数据。State跟每个账户相关，格式是[Hash, Hash]键值对
	// stateObject内部也有类似StateDB一样的二级缓存机制，用来缓存和更新这些state
...
func (self *stateObject) GetStateByteArray(db Database, key common.Hash) []byte {
	...
	fmt.Println("--------------------------------")
	fmt.Printf("Get state byte array value: %v\n", value)
	fmt.Println("--------------------------------")
	return value
}
