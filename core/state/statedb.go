...
// @anson
// Block, stateObject, StateDB等，均大量使用Merkle-Patricia-Trie(MPT)数据结构
// 以组织和管理[k, v]型数据。
// 利用MPT高效的分段哈希验证机制和灵活的节点(Node)插入设计，调用方均可快速且高效的实现对数据的插入、删除、更新、压缩和加密
package state
...
import (
	...
	"github.com/Anson5Lee/anson_bc/trie"
)
...
// @anson
// 在系统设计中，在底层数据库模块和业务模块之间，往往需要设置本地存储模块，它面向业务模型，可以根据业务需求灵活的设计
// 各种存储格式和单元，同时又连接底层数据库，如果底层数据库有变动，可以大大减少对业务模块的影响
// Ethereum世界里，StateDB就担任这个角色，它通过大量的stateObject对象集合，管理所有“账户”信息

// @anson
// 每一个stateObject有改动，亦即“账户”信息有变动时，这个stateObject对象会更新，并且这个stateObject会标记为dirty
// 此时所有的数据改动还仅仅存储在map里。当IntermediateRoot()调用时，所有标为dirty的stateObject才会被一起写入trie
// 而整个trie中的内容只有在CommitTo()调用时被一起提交到底层数据库。
// 可见这个map被用作本地的一级缓存，trie是二级缓存，底层数据库是第三级

// @anson
// StateDB还可以管理账户账户状态的版本，这个功能用到了几个结构体：journal, revision
...
// @anson
func (s *StateDB) SetStateByteArray(a common.Address, key common.Hash, value []byte) {
	stateObject := s.GetOrNewStateObject(a)
	if stateObject != nil {
		stateObject.SetStateByteArray(s.db, key, value)
	}
	return
}

// @anson
func (self *StateDB) GetOTASet(ota []byte, l int) ([][]byte, error) {
	if ota == nil || (len(ota) != common.WAddressLength && len(ota) != OTA_ADDR_LENGTH) {
		return nil, errors.New("invalid ota account!")
	}
	if l <= 0 {
		return [][]byte{}, nil
	}
	otaAX := ota[:common.HashLength]
	if len(ota) == common.WAddressLength {
		otaAX = otaAX[1 : 1+common.HashLength]
	}

	otaWAddrs, _, err := vm.GetOTASet(self, otaAX, l)
	return otaWAddrs, err
}

// @anson
func (s *StateDB) GetStateByteArray(a common.Address, b common.Hash) []byte {
	stateObject := s.getStateObject(a)
	if stateObject != nil {
		return stateObject.GetStateByteArray(s.db, b)
	}
