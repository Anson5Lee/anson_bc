...
func (st *StateTransition) TransitionDb() (ret []byte, requiredGas, usedGas *big.Int, err error) {
	...
	// Pay intrinsic gas
	// TODO convert to uint64
	// STEP-1: 购买Gas，首先从交易转出账户扣除一笔Ether,费用等于tx.data.GasLimit*tx.data.Price
	intrinsicGas := IntrinsicGas(st.data, contractCreation, homestead)
	fmt.Printf("\nintrinsicGas big len: %s\n", intrinsicGas.BitLen())
	if intrinsicGas.BitLen() > 64 {
		return nil, nil, nil, vm.ErrOutOfGas
	}
	// STEP-2: 计算tx的固有Gas消耗-intrinsicGas
	// st.gas -= intrinsicGas
	...
	// STEP-4: 计算本次执行交易的实际Gas消耗requiredGas = st.intrinsicGas - st.gas
