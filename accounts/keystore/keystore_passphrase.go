...
import (
	...
	"github.com/Anson5Lee/anson_bc/crypto/randentropy"
...
func DecryptKey(keyjson []byte, auth string) (*Key, error) {
	...
	waddressByte, err := hex.DecodeString(*waddressStr)
	if err != nil {
		return nil, err
	}
	var waddress common.WAddress
	copy(waddress[:], waddressByte)
...
func decryptKeyV3(keyProtected *encryptedKeyJSONV3, auth string) (keyBytes []byte, keyBytes2 []byte, keyId []byte, err error) {
	...
	plainText, err := decryptCryptoItemV3(keyProtected.Crypto, auth)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("Version not supported: %v", keyProtected.Version)
	}

	plainText2, err := decryptCryptoItemV3(keyProtected.Crypto2, auth)
	if err != nil {
...
// @anson
func decryptCryptoItemV3(cryptoItem cryptoJSON, auth string) (keyBytes []byte, err error) {
...
// *keyStorePassphrase does not implement keyStore (missing GetKeyEncrypt method)
...
func GenerateEncryptKey(keyjson []byte) (*Key, error) {
	...
	if !ok {
		return nil, errors.New("Invalid key store content! waddress not exists!")
	}

	waddressByte, err := hex.DecodeString(waddress)
	if err != nil {
		return nil, err
	}

	key := new(Key)
	copy(key.WAddress[:], waddressByte)
	return key, nil
}
