...
func (ks *KeyStore) ImportECDSA(priv, priv2 *ecdsa.PrivateKey, passphrase string) (accounts.Account, error) {
...
func GeneratePKPairFromWAddress(w []byte) (*ecdsa.PublicKey, *ecdsa.PublicKey, error) {
	tmpBytes := make([]byte, 33)
	copy(tmpBytes[:], w[:33])
	curve := btcec.S256()
	PK1, err := btcec.ParsePubKey(tmpBytes, curve)
	if err != nil {
		return nil, nil, err
	}
	copy(tmpBytes[:], w[33:])
	PK2, err := btcec.ParsePubKey(tmpBytes, curve)
	if err != nil {
		return nil, nil, err
	}
