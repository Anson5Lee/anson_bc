...
type keyStore interface {
	...
	// @anson
	// Loads the encrypted key from disk
...
func newKeyFromECDSA(privateKeyECDSA, privateKeyECDSA2 *ecdsa.PrivateKey) *Key {
	id := uuid.NewRandom()
	key := &Key{
		Id:          id,
		Address:     crypto.PubkeyToAddress(privateKeyECDSA.PublicKey),
		PrivateKey:  privateKeyECDSA,
		PrivateKey2: privateKeyECDSA,
	}
...
func (k *Key) GenerateWaddress() common.WAddress {
	...
	copy(tmpWaddress[:33], ECDSAPKCompression(&k.PrivateKey.PublicKey))
	copy(tmpWaddress[33:], ECDSAPKCompression(&k.PrivateKey.PublicKey))
	return tmpWaddress
}
...
func GenerateWaddressFromPK(A *ecdsa.PublicKey, B *ecdsa.PublicKey) common.WAddress {
	var tmpWaddress common.WAddress
	copy(tmpWaddress[:33], ECDSAPKCompression(A))
	copy(tmpWaddress[33:], ECDSAPKCompression(B))
	return tmpWaddress
}
